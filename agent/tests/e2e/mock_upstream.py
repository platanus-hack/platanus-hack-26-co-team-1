...
<form id="form" method="post" action="{action}" enctype="multipart/form-data">
  <textarea id="prompt" name="prompt" rows="6" cols="60"></textarea>
...
_SITES = {
    "claude.ai": ("Claude (aprobada por la empresa)", "/api/chat"),
    "novaai.local": ("NovaAI", "/api/chat"),
    "asistente-magico.co": ("Asistente Magico", "/v1/chat/completions"),
    "intranet.acme.co": ("Intranet Acme", "/documentos/subir"),
}
        ...
        if flow.response is None:
            ...
            if site is not None:
                ...
                if flow.request.method == "GET":
                    body = _CHAT_PAGE.format(name=name, action=action)
                else:
                    body = _REPLY_PAGE
